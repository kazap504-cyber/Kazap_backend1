 ...
 *
 * Anti-doublon : l'ID du document Firestore est déterministe
 * ("ia-rdv-{callSid}"). Si le même appel est rejoué (même callSid),
 * le document existant est mis à jour au lieu d'être recréé.
 */
async function saveAppointment({ vendorId, callSid, clientName, clientPhone, date, time, motif }) {
  const db = admin.firestore();

  if (!callSid) {
    console.warn('[rdv-booking] saveAppointment appelé sans callSid — anti-doublon désactivé pour ce RDV');
  }

  // Durée de créneau configurée par le vendeur (fallback 60 min)
  let duration = 60;
  try {
    const vendorSnap = await db.collection('vendors').doc(vendorId).get();
    duration = vendorSnap.data()?.settings?.slotDurationMinutes || 60;
  } catch (e) {
  ...
  // ID déterministe si callSid présent → pas de doublon si l'appel est rejoué
  const docId = callSid ? `ia-rdv-${callSid}` : null;
  let ref, created;

  if (docId) {
    ref = db.collection('appointments').doc(docId);
    const existing = await ref.get();
    if (existing.exists) {
      await ref.update(baseData);
      created = false;
    } else {
      await ref.set({ ...baseData, createdAt: admin.firestore.FieldValue.serverTimestamp() });
      created = true;
    }
  } else {
    ref = await db.collection('appointments').add({
      ...baseData,
  ...
  console.log(`✅ [rdv-booking] RDV ${created ? 'créé' : 'mis à jour'} → appointments/${ref.id}`);
  ...
  return { firestoreId: ref.id, created, data: baseData };
}
