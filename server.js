...
const crypto    = require('crypto'); // ← natif Node, utilisé pour signer les uploads Cloudinary
...
// ════════════════════════════════════════════════════════════════════════════
// Avant : le client uploadait directement vers Cloudinary avec un
// "unsigned upload preset" public (CLOUDINARY_CLOUD_NAME + CLOUDINARY_UPLOAD_PRESET
// visibles dans le HTML). N'importe qui connaissant ces deux valeurs pouvait
// envoyer des fichiers arbitraires sur le compte Cloudinary (abus de quota/stockage),
// sans lien avec un vendeur KAZAP identifié.
//
// Maintenant : le client demande une signature ici avant chaque upload. Le
// dossier cible est reconstruit ici, côté serveur, à partir du vendorId reçu
// (jamais accepté tel quel depuis le client) — impossible d'écrire ailleurs
// que dans kazap/vendors/{vendorId}/... Le secret Cloudinary ne quitte jamais
// ce fichier serveur.
...
app.post('/sms/send', async (req, res) => {
  ...
  try {
    ...
    // kazapP10_hasFeature applique la même règle à 'ia', 'agenda' et 'sms' :
    // Basique/trial expiré → pas d'accès. Pro/Premium/essai actif → accès.
    // _accesIAAutorise reproduit cette règle exactement (nom hérité du
    // gating IA plus haut, mais la logique ne dépend pas de la fonctionnalité,
    // seulement du plan/essai — donc valable aussi pour le SMS).
...
// dans index.html). Ajouté pour que le backend redevienne l'autorité finale
// avant d'ouvrir un flux IA vocal coûteux (Twilio + Deepgram + Claude) —
// jusqu'ici seul le toggle voip.iaReceptionEnabled était vérifié ici, jamais
// le plan ni le quota. Ne modifie aucun comportement existant pour un
// vendeur Pro/Premium/essai actif dans les limites de son quota.
// ════════════════════════════════════════════════════════════════════════════
...
/** Reproduit kazapP10_hasFeature('ia', plan) + le patch d'essai (Partie 12) :
 *  Pro/Premium → accès IA. Essai actif (trial/free dans les 14 jours) →
 *  accès Premium complet. Basique, ou essai expiré → pas d'accès IA. */
...
app.post('/call/incoming', async (req, res) => {
  ...
  try {
    ...
    const iaToggleeOn = vendeurData?.voip?.iaReceptionEnabled !== false;

    // ── Gating plan / essai / quota (miroir serveur — voir helpers plus haut) ──
    // Le toggle iaReceptionEnabled ne suffit pas : un vendeur Basique ou dont
    // le quota IA mensuel est épuisé ne doit PAS atteindre Deepgram/Claude,
    // même si le toggle est activé côté dashboard.
    const iaAutorisee = iaToggleeOn && _accesIAAutorise(vendeurData);
    const quotaOk      = iaAutorisee ? await _quotaIADisponible(vendeurId, vendeurData) : false;
    const iaEnabled     = iaToggleeOn && iaAutorisee && quotaOk;

    if (iaToggleeOn && !iaAutorisee) {
      console.warn(`[Gating IA] vendeurId=${vendeurId} — plan insuffisant pour l'IA vocale (appel redirigé).`);
    } else if (iaToggleeOn && iaAutorisee && !quotaOk) {
      console.warn(`[Gating IA] vendeurId=${vendeurId} — quota IA mensuel épuisé (appel redirigé).`);
    }
