...
app.post('/api/cloudinary/sign', async (req, res) => {
  ...
  try {
    let folder;
    if (membreId) {
      // ── Back-office T-Admin : photo de profil d'un membre de l'équipe ──────
      // Dossier dédié, distinct des vendeurs, toujours 'profile' (pas d'autre
      // sous-dossier autorise pour les membres pour l'instant).
      folder = `kazap/membres/${membreId}/profile`;
    } else {
