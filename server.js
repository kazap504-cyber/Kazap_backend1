 ...
 *   POST /api/admin/delete-vendor-account ← T-Admin-9 Suppression d'un compte vendeur par un
 *                                  membre autorise du back-office (Fondatrice ou Developpeur web)
...
const compression = require('compression'); // Optimisation performance : compresse les réponses JSON (gzip) — réduit le temps de transfert, surtout utile sur les connexions mobiles lentes du marché cible
...
app.use(cors());
app.use(compression()); // Gzip sur toutes les réponses — quasi gratuit en CPU, gain net sur les payloads JSON (listes produits/commandes/messages, réponses IA) et sur les connexions lentes
// T-07-F (correctif) : capture du corps brut (rawBody) dans le "verify" de
// express.json(), pour que le webhook FedaPay (plus bas dans ce fichier)
// puisse calculer sa signature HMAC sur les octets exacts reçus, SANS
// dépendre d'un second parseur (express.raw) qui arriverait après coup et
// ne verrait jamais le flux brut (déjà consommé par express.json()).
...
app.post('/api/provision-number', async (req, res) => {
  ...
  try {
    ...
    // [T-VOICE-01] CORRECTIF CRITIQUE — Le numéro acheté n'avait jusqu'ici
    // AUCUNE URL vocale configurée (ni voiceUrl, ni statusCallback) : Twilio
    // ne savait donc pas quoi faire d'un appel entrant, quel que soit le
    // statut (essai ou payant) du compte Twilio. /call/incoming résout déjà
    // le vendeur dynamiquement à partir du numéro appelé (To), donc une seule
    // URL universelle suffit pour tous les vendeurs — pas besoin d'un
    // /call/incoming/:uid par numéro.
    const purchased = await twilioClient.incomingPhoneNumbers.create({
      phoneNumber: numbers[0].phoneNumber,
      voiceUrl: `${process.env.BACKEND_URL}/call/incoming`,
      voiceMethod: 'POST',
      statusCallback: `${process.env.BACKEND_URL}/call/status`,
    ...
    return res.status(500).json({ error: "Erreur lors du provisionnement du numéro." });
  }
});
...
// ── T-DEL03 : suppression des conversations liees a un vendeur ──────────────
// "conversations" n'etait pas dans COLLECTIONS_LIEES_VENDEUR : les fils de
// discussion (et leurs messages, en sous-collection conversations/{id}/messages)
// d'un vendeur supprime restaient donc orphelins en base. Chaque conversation
// est supprimee avec db.recursiveDelete() pour emporter sa sous-collection
// "messages" avec elle (un simple .delete() sur le document parent ne supprime
// pas les sous-collections).
// A CONFIRMER, comme pour COLLECTIONS_LIEES_VENDEUR ci-dessus : le nom du champ
// ("vendorId") est suppose par coherence avec products/orders/appointments,
// mais n'a pas ete verifie contre le schema reel cote index.html/boutique.html.
// Si le champ reel porte un autre nom, la requete ne trouvera simplement aucun
// document (aucun risque de supprimer autre chose) — mais le nettoyage restera
// inoperant tant que ce n'est pas aligne.
async function supprimerConversationsVendeur(uid) {
  const snap = await db.collection('conversations').where('vendorId', '==', uid).get();
  if (snap.empty) return;

  for (const doc of snap.docs) {
    await db.recursiveDelete(doc.ref);
  }
  console.log(`[delete-account] ${snap.docs.length} conversation(s) (+ messages) supprimee(s) pour uid=${uid}`);
}
...
// ── Suppression complete d'un compte vendeur (Firestore + Twilio + Auth) ────
// Factorisee pour etre appelee soit par le vendeur lui-meme (/api/delete-account),
// soit par un membre autorise du back-office (/api/admin/delete-vendor-account).
// Ne fait aucune verification d'autorisation : c'est la responsabilite de
// l'appelant (route Express) de verifier qui a le droit de declencher ceci.
async function supprimerCompteVendeurComplet(uid) {
  try {
  ...
  await supprimerConversationsVendeur(uid);
...
app.post('/api/delete-account', async (req, res) => {
  ...
  try {
    await supprimerCompteVendeurComplet(uid);
  } catch (err) {
    console.error('[delete-account] Erreur suppression :', err.message);
    return res.status(500).json({
      error: 'Erreur lors de la suppression du compte. Reessayez ou contactez le support.'
    });
  }
...
// ════════════════════════════════════════════════════════════════════════════
// T-Admin-9 : Suppression d'un compte vendeur INACTIF depuis le back-office
// Utilisee par la section "Alertes Techniques", reservee aux membres ayant le
// role 'fondatrice' ou la fonction 'Développeur web' dans la collection
// Firestore "membres". Contrairement a /api/delete-account (qui n'autorise que
// l'auto-suppression), cette route permet a un membre autorise de supprimer le
// compte d'un AUTRE utilisateur (un vendeur abandonne). Additif : ne modifie
// aucune route existante.
// ════════════════════════════════════════════════════════════════════════════
app.post('/api/admin/delete-vendor-account', async (req, res) => {
  const { uid } = req.body;
  if (!uid) return res.status(400).json({ error: 'uid requis' });
  ...
  try {
    ...
    console.error('[admin-delete-vendor] Token invalide :', err.message);
    return res.status(401).json({ error: 'Token d\'authentification invalide' });
  }

  // ── Verification que l'appelant est un membre autorise du back-office ─────
  let membreSnap;
  try {
    membreSnap = await db.collection('membres').doc(decodedToken.uid).get();
  } catch (err) {
    console.error('[admin-delete-vendor] Erreur lecture membre :', err.message);
    return res.status(500).json({ error: 'Verification des droits impossible' });
  }

  if (!membreSnap.exists) {
    return res.status(403).json({ error: 'Compte non autorise' });
  }
  const membre = membreSnap.data();
  const autorise = membre.role === 'fondatrice' || membre.fonction === 'Développeur web';
  if (!autorise) {
    console.warn(`[admin-delete-vendor] Membre ${decodedToken.uid} (fonction=${membre.fonction}, role=${membre.role}) non autorise`);
    return res.status(403).json({ error: 'Reserve a la Fondatrice ou au Developpeur web' });
  }

  try {
    await supprimerCompteVendeurComplet(uid);
  } catch (err) {
    console.error('[admin-delete-vendor] Erreur suppression :', err.message);
    return res.status(500).json({
      error: 'Erreur lors de la suppression du compte. Reessayez ou contactez le support.'
    });
  }

  console.log(`[admin-delete-vendor] Compte vendeur ${uid} supprime par membre ${decodedToken.uid} (${membre.nom || membre.email})`);
  return res.status(200).json({ success: true });
});
...
/** Allocation unique de l'essai gratuit de 14 jours (non mensuelle, non
 *  renouvelable) : minutes d'appel IA, SMS, réponses IA à la messagerie.
 *  Doit rester synchronisée avec window.kazapP15_TRIAL_LIMITS dans index.html. */
const KAZAP_TRIAL_LIMITS = { callMinutes: 10, sms: 20, aiReplies: 10 };
...
/** Quotas de l'essai gratuit (14 jours) — additif, ne modifie aucune des
 *  fonctions ci-dessus. Somme les durées d'appel IA depuis le DÉBUT DE
 *  L'ESSAI (planActivatedAt, ou createdAt en repli — même référence que
 *  _essaiEncoreActif), et non depuis le début du mois civil : l'allocation
 *  d'essai est unique pour toute la période de 14 jours, non renouvelable. */
async function _minutesIAUtiliseesEssai(vendeurId, vendeurData) {
  const debutEssaiMs = _tsEnMs(vendeurData?.planActivatedAt) || _tsEnMs(vendeurData?.createdAt);
  const debutEssai   = debutEssaiMs ? new Date(debutEssaiMs) : new Date(0);

  const snap = await db.collection('ia_orders')
    .where('vendeurId', '==', vendeurId)
    .where('createdAt', '>=', debutEssai)
    .get();
...
async function _quotaIADisponible(vendeurId, vendeurData) {
  if (_compteEstBloque(vendeurData)) return false;
  if (_essaiEncoreActif(vendeurData)) {
    try {
      const minutesUtilisees = Math.ceil(await _minutesIAUtiliseesEssai(vendeurId, vendeurData));
      return minutesUtilisees < KAZAP_TRIAL_LIMITS.callMinutes;
    } catch (err) {
      console.error('[Quota essai — appels] Erreur lecture ia_orders :', err.message);
      return true;
    }
  }
  const plan   = _normaliserPlan(vendeurData);
...
async function _quotaIADepasseEnCours(vendeurId, vendeurData, elapsedSec) {
  ...
  if (_essaiEncoreActif(vendeurData)) {
    try {
      const minutesAvant = await _minutesIAUtiliseesEssai(vendeurId, vendeurData);
      const minutesTotal = minutesAvant + (Number(elapsedSec) || 0) / 60;
      return minutesTotal >= KAZAP_TRIAL_LIMITS.callMinutes;
    } catch (err) {
      console.error('[Quota essai — appels en cours] Erreur lecture ia_orders :', err.message);
      return false;
    }
  }
  const plan   = _normaliserPlan(vendeurData);
...
/** Additif — quota SMS de l'essai gratuit (20 SMS, allocation unique pour
 *  toute la période de 14 jours, non mensuelle). En dehors de l'essai, le
 *  comportement existant est préservé à l'identique : aucune limite
 *  numérique de SMS n'existe pour les plans payants (seul _accesIAAutorise
 *  gère l'accès à la fonctionnalité elle-même), donc cette fonction renvoie
 *  toujours true hors essai — non-régression garantie pour Basique/Pro/
 *  Premium. */
function _quotaSMSDisponible(vendeurData) {
  if (_compteEstBloque(vendeurData)) return false;
  if (!_essaiEncoreActif(vendeurData)) return true;
  const smsUtilises = Number(vendeurData?.trialUsage?.sms || 0);
  return smsUtilises < KAZAP_TRIAL_LIMITS.sms;
}

/** Additif — quota réponses IA écrites (messagerie) de l'essai gratuit
 *  (10 réponses, allocation unique). Hors essai, comportement existant
 *  préservé à l'identique (toujours true — aucune limite numérique
 *  n'existait avant pour les plans payants). */
function _quotaReponseIADisponible(vendeurData) {
  if (_compteEstBloque(vendeurData)) return false;
  if (!_essaiEncoreActif(vendeurData)) return true;
  const reponsesUtilisees = Number(vendeurData?.trialUsage?.aiReplies || 0);
  return reponsesUtilisees < KAZAP_TRIAL_LIMITS.aiReplies;
}

/** Additif — incrémente un compteur d'usage de l'essai gratuit
 *  (trialUsage.sms ou trialUsage.aiReplies) de façon atomique. Ne fait rien
 *  hors essai actif (les plans payants n'ont pas de compteur trialUsage à
 *  tenir à jour). Erreurs journalisées mais jamais bloquantes : un échec
 *  d'incrémentation ne doit jamais faire échouer l'action déjà effectuée
 *  (SMS déjà envoyé / réponse IA déjà générée). */
async function _incrementerUsageEssai(vendeurId, vendeurData, champ) {
  if (!_essaiEncoreActif(vendeurData)) return;
  try {
    await db.collection('vendors').doc(vendeurId).update({
      [`trialUsage.${champ}`]: admin.firestore.FieldValue.increment(1)
    });
  } catch (err) {
    console.error(`[Quota essai] Échec incrémentation trialUsage.${champ} — vendorId=${vendeurId} :`, err.message);
  }
}
...
app.post('/api/cloudinary/sign', async (req, res) => {
  const { vendorId, membreId, sousDossier } = req.body;
  if (!vendorId && !membreId) return res.status(400).json({ error: 'vendorId ou membreId requis' });
  ...
  try {
    let folder;
    if (membreId) {
      // ── Back-office T-Admin : fichiers d'un membre de l'équipe (photo de
      // profil ou message vocal). Sous-dossier limité à une liste connue.
      const sousDossiersMembreAutorises = ['profile', 'vocaux'];
      const dossierMembre = sousDossiersMembreAutorises.includes(sousDossier) ? sousDossier : 'profile';
      folder = `kazap/membres/${membreId}/${dossierMembre}`;
    } else {
      // Sous-dossier limité à une liste connue pour éviter toute injection
      // de chemin (ex. "../../autre-vendeur") — 'products' par défaut.
      // 'messages' : pièces jointes de la messagerie (images/documents/vidéos),
      // ajouté pour la messagerie écrite (menu "+" > pièce jointe).
      const sousDossiersAutorises = ['products', 'shop', 'profile', 'messages'];
      const dossier = sousDossiersAutorises.includes(sousDossier) ? sousDossier : 'products';
      folder = `kazap/vendors/${vendorId}/${dossier}`;
    }
    ...
    return res.status(500).json({ error: "Erreur lors de la préparation de l'upload." });
  }
});
...
app.post('/sms/send', verifierCompteActif, async (req, res) => {
  ...
  try {
    ...
    // ── Quota SMS de l'essai gratuit (20 SMS, allocation unique) ──
    if (!_quotaSMSDisponible(vendor)) {
      console.warn(`[Quota essai — SMS] vendorId=${vendorId} — plafond de ${KAZAP_TRIAL_LIMITS.sms} SMS atteint.`);
      return res.status(403).json({ error: "Le quota de SMS inclus dans votre essai gratuit est atteint. Passez à un plan supérieur pour continuer." });
    }
    ...
    await _incrementerUsageEssai(vendorId, vendor, 'sms');
...
app.post('/webhooks/twilio/:vendorId/no-answer', async (req, res) => {
  ...
  // [T-VOICE-03] Appel manqué (transfert sans réponse) — jusqu'ici totalement
  // silencieux côté vendeur, contrairement au système IA qui notifie déjà ses
  // commandes/RDV. Best-effort : ne doit jamais faire échouer la réponse TwiML.
  sendFCMPush(
    vendorId,
    '📞 Appel manqué',
    req.body.From ? `Appel manqué de ${req.body.From}` : 'Vous avez un appel manqué',
    { type: 'missed_call', clickUrl: '/dashboard?section=ia' }
  ).catch(() => {});
  return res.set('Content-Type', 'text/xml').send(`
    <Response>
...
app.post('/webhooks/twilio/:vendorId/recording', async (req, res) => {
  ...
  // [T-VOICE-03] Même correctif que pour l'appel manqué ci-dessus : un message
  // vocal laissé était jusqu'ici silencieux côté vendeur. Best-effort.
  sendFCMPush(
    vendorId,
    '🎙️ Nouveau message vocal',
    `De ${callerNumber}`,
    { type: 'voicemail', clickUrl: '/dashboard?section=ia' }
  ).catch(() => {});
...
async function sendFCMPush(vendorId, title, body, data = {}) {
  try {
    ...
    const message = {
      ...
      webpush: {
        notification: {
          // T-NOTIF-ICON-FIX — icon/badge pointaient vers "votre-domaine.com",
          // un placeholder jamais remplacé (URL invalide, jamais résolue).
          // Certains navigateurs (Brave notamment) peuvent annuler tout
          // l'affichage de la notification si l'icône déclarée est
          // introuvable. On retire ces champs optionnels plutôt que de
          // laisser une URL cassée — le navigateur utilisera son icône
          // par défaut, ce qui est plus sûr.
          vibrate: [200, 100, 200]
        }
      }
    };
...
// ── Nouvelle commande boutique publique — Notification FCM (T-NOTIF-ORDER) ──
// Calqué sur /api/notify-message ci-dessus. Appelé par boutique.html juste après
// l'addDoc de la commande dans Firestore, en best-effort (ne bloque jamais la
// commande côté client). Permet au vendeur de recevoir un push même onglet fermé.
app.post('/api/notify-order', async (req, res) => {
  const { vendorId, orderId, clientName, productName, total } = req.body;
  if (!vendorId || !orderId) {
    return res.status(400).json({ error: 'vendorId et orderId requis' });
  }
  try {
    const totalStr = Number(total || 0).toLocaleString('fr-FR');
    let body = `${clientName || 'Un client'}${productName ? ' — ' + productName : ''}`;
    if (totalStr && totalStr !== '0') body += ` (${totalStr} FCFA)`;
    ...
    console.error('[Notify Order] Erreur :', err.message);
    return res.status(500).json({ error: "Erreur lors de l'envoi de la notification." });
  }
});
...
app.post('/api/ia-auto-reply-message', verifierCompteActif, async (req, res) => {
  ...
  try {
    ...
    // ── Quota réponses IA de l'essai gratuit (10 réponses, allocation unique) ──
    if (!_quotaReponseIADisponible(vendeurData)) {
      console.warn(`[Quota essai — réponses IA] vendorId=${vendorId} — plafond de ${KAZAP_TRIAL_LIMITS.aiReplies} réponses atteint.`);
      return res.json({ success: false, reason: "Le quota de réponses IA inclus dans votre essai gratuit est atteint. Passez à un plan supérieur pour continuer." });
    }
    ...
    await _incrementerUsageEssai(vendorId, vendeurData, 'aiReplies');
...
app.post('/call/incoming', async (req, res) => {
  ...
  try {
    ...
    // [T-VOICE-02] Aiguillage IA vocale ↔ menu DTMF classique, au choix du
    // vendeur (voip.callSystem : 'ia' par défaut, ou 'dtmf'). Un numéro Twilio
    // n'a qu'une seule URL vocale possible ; /call/incoming sert donc de point
    // d'entrée unique pour tous les vendeurs, et redirige vers le système
    // classique existant (/webhooks/twilio/:vendorId, transfert + menu
    // horaires/messagerie) sans dupliquer sa logique ni la modifier.
    if (vendeurData?.voip?.callSystem === 'dtmf') {
      const twimlRedirect = new VoiceResponse();
      twimlRedirect.redirect({ method: 'POST' }, `${process.env.BACKEND_URL}/webhooks/twilio/${vendeurId}`);
      res.type('text/xml');
      return res.send(twimlRedirect.toString());
    }
...
app.post('/api/voice/toggle', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Erreur lors de la mise à jour du réglage." });
  }
});

// [T-VOICE-02] Choix du système d'appel : 'ia' (assistant vocal) ou 'dtmf'
// (menu classique horaires/messagerie) — lu par /call/incoming ci-dessus.
app.post('/api/voice/call-system', async (req, res) => {
  const { vendorId, callSystem } = req.body;
  if (!vendorId || !['ia', 'dtmf'].includes(callSystem)) {
    return res.status(400).json({ error: "vendorId et callSystem ('ia' ou 'dtmf') requis" });
  }
  try {
    await db.collection('vendors').doc(vendorId).update({
      'voip.callSystem': callSystem,
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
    res.json({ success: true, callSystem });
  } catch (err) {
    console.error('POST /api/voice/call-system erreur :', err.message);
    res.status(500).json({ error: "Erreur lors de la mise à jour du réglage." });
  }
});
...
function _demarrerControleQuotaEnCours(session, vendeurId) {
  ...
  const estEssai = _essaiEncoreActif(vendeurData);
  if (!estEssai && _normaliserPlan(vendeurData) !== 'pro') return; // 'premium' illimité
  ...
  session.quotaCheckInterval = setInterval(async () => {
    ...
    try {
      ...
      if (depasse) {
        ...
        if (estEssai) {
          console.warn(`[Quota essai] Limite de ${KAZAP_TRIAL_LIMITS.callMinutes} min atteinte en cours d'appel — vendeurId=${vendeurId} callSid=${session.callSid}`);
        } else {
        ...
        await _couperAppelQuotaAtteint(session, vendeurId, estEssai);
      }
    } catch (err) {
 ...
 * Termine proprement un appel dès que le quota de minutes IA est atteint
 * PENDANT l'appel (essai gratuit : 10 min pour toute la période de 14 jours ;
 * Pro : 60 min/mois) : informe l'appelant par un message vocal court, puis
 ...
 * @param {object} session   — session WebSocket media-stream en cours
 * @param {string} vendeurId
 * @param {boolean} [estEssai] — true si le plafond atteint est celui de
 *   l'essai gratuit (10 min, non renouvelable) plutôt que le quota mensuel
 *   Pro (60 min/mois). Change uniquement le message dit à l'appelant et le
 *   détail enregistré ; le raccrochage est identique dans les deux cas.
 */
async function _couperAppelQuotaAtteint(session, vendeurId, estEssai) {
  const msgFin = estEssai
    ? "Je suis désolée, les 10 minutes d'appel incluses dans votre essai gratuit ont été atteintes. Passez à un plan supérieur pour continuer à profiter de l'assistante vocale. Au revoir."
    : "Je suis désolée, la durée d'utilisation de l'assistante vocale pour ce mois a été atteinte. Le service redevient disponible le mois prochain. Vous pouvez aussi passer à un plan supérieur. Au revoir.";
  ...
  try {
    if (session.callSid) {
      ...
      console.log(`Appel ${session.callSid} raccroché proprement (${estEssai ? "quota d'essai gratuit atteint" : 'quota IA Pro atteint'}).`);
    }
  } catch (err) {
    ...
    try {
      if (session.callSid) {
        await mettreAJourStatutAppel(session.callSid, {
          ...
          details: estEssai
            ? "Appel interrompu automatiquement : quota d'essai gratuit (10 min, non renouvelable) atteint pendant l'appel."
            : "Appel interrompu automatiquement : quota d'appel IA Pro (60 min/mois) atteint pendant l'appel.",
...
app.post('/webhook/fedapay', async (req, res) => {
  ...
  try {
    ...
    const rawBody      = req.rawBody; // Buffer brut capturé par verify() de express.json()
    const sigHeader    = req.headers['x-fedapay-signature'] || '';

    if (!rawBody) {
      console.error('[T-07-F] req.rawBody manquant — vérifier que express.json({verify}) capture bien le corps');
      clearTimeout(_safetyTimeout);
      return _respond(400, { error: 'Corps de requête manquant' });
    }
    ...
    // ── ÉTAPE 2 : Validation de l'événement (déjà parsé par express.json()) ─
    const event = req.body;
    if (!event || typeof event !== 'object') {
      console.error('[T-07-F] Corps JSON invalide ou vide');
      clearTimeout(_safetyTimeout);
    ...
    // ── ÉTAPE 6bis : Journalisation immuable du paiement (back-office T-Admin-5)
    // Contrairement à vendors/{uid}, ce document n'est jamais écrasé : chaque
    // paiement approuvé crée une nouvelle ligne. C'est ce qui permet au
    // back-office de construire un vrai journal des paiements et un calcul
    // de chiffre d'affaires fiable (là où planAmountPaidFCFA ne reflète que
    // le DERNIER paiement du vendeur). Ajout additif : n'affecte rien
    // d'existant, aucune lecture ni écriture de ce fichier n'est modifiée.
    await db.collection('paiements').add({
      vendorId:    vendorUid,
      vendorNom:   vendorData?.boutiqueName
                     || `${vendorData?.prenom || ''} ${vendorData?.nom || ''}`.trim()
                     || null,
