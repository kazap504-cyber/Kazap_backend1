 ...
 *   SMTP_HOST                → Hôte du serveur SMTP (ex: smtp.sendgrid.net) — requis pour /otp/send-email
 *   SMTP_PORT                → Port SMTP (ex: 587). Optionnel, défaut 587.
 *   SMTP_USER                → Identifiant SMTP
 *   SMTP_PASS                → Mot de passe / clé API SMTP
 *   SMTP_FROM_EMAIL          → Adresse expéditeur affichée (défaut : SMTP_USER)
 *
 * ROUTES :
 *   POST /sms/send
 *   POST /otp/send
...
const nodemailer = require('nodemailer'); // OTP par email (route /otp/send-email)
...
// ─── SMTP (OTP par email) ──────────────────────────────────────────────────
// Transporteur créé une seule fois et réutilisé (pool de connexions).
// Retourne null si les variables SMTP_HOST/SMTP_USER/SMTP_PASS ne sont pas
// configurées, pour que /otp/send-email échoue proprement au lieu de crasher.
...
app.post('/otp/send-email', async (req, res) => {
  ...
  if (!transporter) {
    console.error('[OTP Send Email] Configuration SMTP manquante (SMTP_HOST / SMTP_USER / SMTP_PASS)');
