 ...
 * @returns {Promise<{reponse: string, type: string, details: Object, statutCommande: (string|null)}|null>}
 *          — null si le plan du vendeur n'autorise pas l'IA écrite (Partie 5/8, Volet 3/4)
 */
async function genererReponseIAEcrit({ message, vendeurData, historique = [], lang = 'fr', tonalite = null }) {
  // ── Gating plan (ajout — Partie 5/8, Volet 3/4) ─────────────────────────
  // Réutilise _accesIAAutorise (server.js, Partie 3/8) plutôt que de
  // recréer une logique de vérification du plan : Basique (et trial/free,
  // alignés sur Basique en miroir de kazapP10_hasFeature('ia') côté client)
  // → réponse IA écrite non autorisée. Pro/Premium → inchangé.
  // Import différé (à l'intérieur de la fonction, et non en haut du fichier)
  // pour éviter toute dépendance circulaire avec server.js, qui require
  // déjà ce module (claude-agent.js) au chargement.
  const { _accesIAAutorise } = require('./server');
  if (!_accesIAAutorise(vendeurData)) {
    console.log('🔒 [messagerie IA] Accès IA non autorisé pour ce plan — aucune réponse générée.');
    return null;
  }
